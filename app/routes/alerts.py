...
async def create_alert_endpoint(alert: AlertCreate, db: Session = Depends(get_db)):
    car = get_car_by_id(db, alert.car_id)
    if not car:
        raise HTTPException(
...
async def get_alerts(
    ...
    return get_alerts_by_car(db, car_id, skip=skip, limit=limit, unread_only=unread_only)


@router.patch("/{alert_id}/read", response_model=AlertResponse)
async def mark_alert_as_read(alert_id: int, db: Session = Depends(get_db)):
    alert = mark_as_read(db, alert_id)
    if not alert:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def mark_all_alerts_as_read(car_id: int, db: Session = Depends(get_db)):
    car = get_car_by_id(db, car_id)
    if not car:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    count = mark_all_as_read(db, car_id)
